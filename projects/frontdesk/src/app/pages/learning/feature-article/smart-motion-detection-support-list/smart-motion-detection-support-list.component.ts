...
import { SmartMotionDetectionSupportModel } from '@frontdesk/core/interfaces/smart-motion-detection-support-model';
import { ActivatedRoute, Router } from '@angular/router';
import { I18nService } from '@frontdesk/core/services';

@Component({
  selector: 'vvtk-smart-motion-detection-support-list',
  templateUrl: './smart-motion-detection-support-list.component.html',
  styleUrls: ['./smart-motion-detection-support-list.component.scss']
})
export class SmartMotionDetectionSupportListComponent implements OnInit {

  supportModels: SmartMotionDetectionSupportModel[] = [];
  filteredSupportModels: SmartMotionDetectionSupportModel[] = [];
  groupedSupportModels: Map<number, SmartMotionDetectionSupportModel[]> = new Map<number, SmartMotionDetectionSupportModel[]>();
  ...
  getTemplate() {
    this.vvtkService.get(
      {
        path: 'api/Products/SmartMotionDetection/SupportPage'
      },
      {
        next: resp => {
  ...
  getData() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartMotionDetection/Support`,
  ...
  getFilter() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartMotionDetection/Support/Filter/ModelType`,
        disableLanguage: true
      },
      {
        next: resp => {
    ...
    // this.vvtkService.get(
    //   {
    //     path: `api/Products/SmartMotionDetection/Support/Filter/ModelStatus`,
    //     disableLanguage: true
    //   },
    //   {
    //     next: resp => {
    ...
    this.vvtkService.get(
      {
        path: `api/Products/SmartMotionDetection/Support/Filter/ModelFeature`,
        disableLanguage: true
      },
      {
        next: resp => {
  ...
  groupSupportModel() {
    const groupModels: Map<number, SmartMotionDetectionSupportModel[]> = new Map<number, SmartMotionDetectionSupportModel[]>();

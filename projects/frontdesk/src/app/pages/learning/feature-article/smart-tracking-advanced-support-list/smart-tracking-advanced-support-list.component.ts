...
import { SmartTrackingAdvancedSupportModel } from '@frontdesk/core/interfaces/smart-tracking-advanced-support-mode';
import { ActivatedRoute, Router } from '@angular/router';
import { I18nService } from '@frontdesk/core/services';

@Component({
  selector: 'vvtk-smart-tracking-advanced-support-list',
  templateUrl: './smart-tracking-advanced-support-list.component.html',
  styleUrls: ['./smart-tracking-advanced-support-list.component.scss']
})
export class SmartTrackingAdvancedSupportListComponent implements OnInit {

  supportModels: SmartTrackingAdvancedSupportModel[] = [];
  filteredSupportModels: SmartTrackingAdvancedSupportModel[] = [];
  groupedSupportModels: Map<number, SmartTrackingAdvancedSupportModel[]> = new Map<number, SmartTrackingAdvancedSupportModel[]>();
  ...
  getTabs() {
    ...
    this.vvtkService.get(
      {
        path: 'api/SmartTrackingAdvanced'
      },
      {
        next: resp => {
  ...
  getTemplate() {
    this.vvtkService.get(
      {
        path: 'api/Products/SmartTrackingAdvanced/SupportPage'
      },
      {
        next: resp => {
  ...
  getData() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartTrackingAdvanced/Support`,
  ...
  getFilter() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartTrackingAdvanced/Support/Filter/ModelType`,
        disableLanguage: true
      },
      {
        next: resp => {
    ...
    // this.vvtkService.get(
    //   {
    //     path: `api/Products/SmartTrackingAdvanced/Support/Filter/ModelStatus`,
    //     disableLanguage: true
    //   },
    //   {
    //     next: resp => {
    ...
    this.vvtkService.get(
      {
        path: `api/Products/SmartTrackingAdvanced/Support/Filter/ModelFeature`,
        disableLanguage: true
      },
      {
        next: resp => {
  ...
  groupSupportModel() {
    const groupModels: Map<number, SmartTrackingAdvancedSupportModel[]> = new Map<number, SmartTrackingAdvancedSupportModel[]>();

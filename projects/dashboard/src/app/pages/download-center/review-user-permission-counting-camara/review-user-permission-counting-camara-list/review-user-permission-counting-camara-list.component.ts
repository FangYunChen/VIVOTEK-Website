...
import {
  ReviewUserPermissionCountingCamaraContentComponent, ReviewUserPermissionData
} from '../review-user-permission-counting-camara-content/review-user-permission-counting-camara-content.component';
import { DownloadCenterApplication } from 'projects/dashboard/src/app/vvtk-core/interface/download-center';

@Component({
  selector: 'vvtk-review-user-permission-counting-camara-list',
  templateUrl: './review-user-permission-counting-camara-list.component.html'
})
export class ReviewUserPermissionCountingCamaraListComponent implements OnInit {
  ...
  private getApplyPermissionUsers() {
    ...
    this.vvtkApiService.get<UserBasicModel[]>({
      path: `api/DownloadCenter/Reviewers/Applicants/CountingCameraFirmware`,
      ...
      .subscribe(
        permissions => {
          ...
          const dialogRef = this.dialog
            .open<ReviewUserPermissionCountingCamaraContentComponent, ReviewUserPermissionData, DownloadCenterApplication[]>(
  ...
  private getUserApplyPermissions(userId: string) {
    return this.vvtkApiService.get<DownloadCenterApplication[]>({
      path: `api/DownloadCenter/Applicants/CountingCameraFirmware/${userId}`,
      disableLanguage: true
    });
  }

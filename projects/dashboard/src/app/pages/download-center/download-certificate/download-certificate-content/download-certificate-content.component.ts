...
@Component({
  selector: 'vvtk-download-certificate-content',
  templateUrl: './download-certificate-content.component.html'
})
export class DownloadCertificateContentComponent implements OnInit, OnDestroy {

  pageIsEditable: boolean;
  pageCertificate: boolean;
  pagedocumentTypeId: number;
  ...
  ngOnInit() {
    ...
    this.pageCertificate = true;
    this.pagedocumentTypeId = 3;
  ...
  checkIsProductDocumentType(documentTypeId: number): boolean {
    if (documentTypeId === 3 || documentTypeId === 36) {
      return true;
    } else {
  ...
  save() {
    ...
    const save$ = this.isAddDownload ?
      ...
      : this.vvtkApiService.patch({
        path: `api/DownloadCenter/Mains/Certificate/${this.id}`,
      }, this.postData);
    save$.pipe(
      finalize(() => this.isLoading = false)
    ).subscribe(
      _ => this.router.navigate(['/download-center/download-certificate'], {queryParams: {selectT: this.selectTTemp, selectP: this.selectPTemp}})
    );
  }

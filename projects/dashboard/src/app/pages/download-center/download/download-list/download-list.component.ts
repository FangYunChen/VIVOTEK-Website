...
export class DownloadListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    combineLatest(
      selectedLanguage$,
      selectedDocumentTypeValueChange$,
      selectedProductValueChange$,
      sortChange$,
      paginationChange$
    ).subscribe(([lang, documentTypeId, productId, sort, page]: [string, number, number, Sort, PageEvent]) => {
      this.reloadData(documentTypeId, productId, sort, page);
    });
  }

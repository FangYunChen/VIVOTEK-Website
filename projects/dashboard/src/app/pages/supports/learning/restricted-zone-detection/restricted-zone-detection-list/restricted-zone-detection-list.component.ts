...
@Component({
  ...
  templateUrl: './restricted-zone-detection-list.component.html',
  styleUrls: ['./restricted-zone-detection-list.component.scss']
})
export class RestrictedZoneDetectionListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/supports/learning/restricted-zone-detection/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/RestrictedZoneDetection/ManageList`
      },
      {
        next: resp => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
        ...
        this.vvtkService.delete(
          {
            path: `api/RestrictedZoneDetection/${id}`,
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/RestrictedZoneDetection/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

...
import {
  RestrictedZoneDetectionSupport,
  SupportType,
  SupportFeature
} from '../../../../../../../vvtk-core/classes/support-restricted-zone-detection';
...
export class SupportModelContentComponent implements OnInit {
  ...
  id: number;
  support: RestrictedZoneDetectionSupport;
  path = 'RestrictedZoneDetectionSupportModel';
  types: SupportType[] = [];
  ...
  getSupportModel() {
    this.vvtkService.get(
      {
        path: `api/Products/RestrictedZoneDetection/Support/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        ...
        {
          next: resp => {
            this.router.navigate(['/supports/learning/restricted-zone-detection/support/model']);
          },
          finally: () => {
            this.isLoading = false;
          }

...
import { SupportTypeManage } from '../../../../../../../vvtk-core/classes/support-restricted-zone-detection';
...
export class ModelTypeListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/supports/learning/restricted-zone-detection/support/type/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/Products/RestrictedZoneDetection/Support/Type/ManageList`,
    ...
    dialogRef.afterClosed().subscribe((result: DeleteConfirmResult) => {
      if (result && result.value) {
        ...
        this.vvtkService.delete(
          {
            path: `api/Products/RestrictedZoneDetection/Support/Type/${id}/${result.value}`,
            disableLanguage: true
          },
          {
            next: resp => {
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/Products/RestrictedZoneDetection/Support/Type/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

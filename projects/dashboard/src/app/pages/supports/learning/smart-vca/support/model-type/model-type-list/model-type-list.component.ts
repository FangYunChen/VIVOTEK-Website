...
import { SupportTypeManage } from '../../../../../../../vvtk-core/classes/support-smart-vca';
...
export class ModelTypeListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/supports/learning/smart-vca/support/type/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/Products/SmartVca/Support/Type/ManageList`,
    ...
    dialogRef.afterClosed().subscribe((result: DeleteConfirmResult) => {
      if (result && result.value) {
        ...
        this.vvtkService.delete(
          {
            path: `api/Products/SmartVca/Support/Type/${id}/${result.value}`,
            disableLanguage: true
          },
          {
            next: resp => {
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/Products/SmartVca/Support/Type/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

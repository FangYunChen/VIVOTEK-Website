...
import { SupportPackage } from '../../../../../../../vvtk-core/classes/support-smart-vca';
...
export class PackageContentComponent implements OnInit {
  ...
  getPackage() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartVca/Support/Package/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        {
          path: `api/Products/SmartVca/Support/Package`,
          disableLanguage: true
        },
        this.package,
        {
          next: resp => {
            this.router.navigate(['/supports/learning/smart-vca/support/package']);
          },
          finally: () => {

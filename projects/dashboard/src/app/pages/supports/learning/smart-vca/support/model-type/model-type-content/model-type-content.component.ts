...
import { SupportType } from '../../../../../../../vvtk-core/classes/support-smart-vca';
...
export class ModelTypeContentComponent implements OnInit {
  ...
  getModelType() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartVca/Support/Type/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        {
          path: `api/Products/SmartVca/Support/Type`,
          disableLanguage: true
        },
        this.type,
        {
          next: resp => {
            this.router.navigate(['/supports/learning/smart-vca/support/type']);
          },
          finally: () => {

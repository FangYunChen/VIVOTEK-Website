...
import {
  ...
  SupportFeature,
  SupportLicense
} from '../../../../../../../vvtk-core/classes/support-smart-vca';
...
export class SupportModelContentComponent implements OnInit {
  ...
  id: number;
  support: SmartVcaSupport;
  path = 'SmartVcaSupportModel';
  types: SupportType[] = [];
  ...
  getSupportModel() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartVca/Support/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        ...
        {
          next: resp => {
            this.router.navigate(['/supports/learning/smart-vca/support/model']);
          },
          finally: () => {
            this.isLoading = false;
          }

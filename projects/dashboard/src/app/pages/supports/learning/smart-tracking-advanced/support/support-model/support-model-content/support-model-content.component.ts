...
import {
  SmartTrackingAdvancedSupport,
  SupportType,
  SupportFeature
} from '../../../../../../../vvtk-core/classes/support-smart-tracking-advanced';
...
export class SupportModelContentComponent implements OnInit {
  ...
  id: number;
  support: SmartTrackingAdvancedSupport;
  path = 'SmartTrackingAdvancedSupportModel';
  types: SupportType[] = [];
  ...
  getSupportModel() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartTrackingAdvanced/Support/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        ...
        {
          next: resp => {
            this.router.navigate(['/supports/learning/smart-tracking-advanced/support/model']);
          },
          finally: () => {
            this.isLoading = false;
          }

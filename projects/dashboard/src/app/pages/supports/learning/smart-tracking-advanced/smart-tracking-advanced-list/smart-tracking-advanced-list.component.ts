...
@Component({
  ...
  templateUrl: './smart-tracking-advanced-list.component.html',
  styleUrls: ['./smart-tracking-advanced-list.component.scss']
})
export class SmartTrackingAdvancedListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/supports/learning/smart-tracking-advanced/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/SmartTrackingAdvanced/ManageList`
      },
      {
        next: resp => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
        ...
        this.vvtkService.delete(
          {
            path: `api/SmartTrackingAdvanced/${id}`,
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/SmartTrackingAdvanced/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

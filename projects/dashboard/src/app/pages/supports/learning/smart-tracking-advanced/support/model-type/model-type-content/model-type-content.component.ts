...
import { SupportType } from '../../../../../../../vvtk-core/classes/support-smart-tracking-advanced';
...
export class ModelTypeContentComponent implements OnInit {
  ...
  getModelType() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartTrackingAdvanced/Support/Type/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        {
          path: `api/Products/SmartTrackingAdvanced/Support/Type`,
          disableLanguage: true
        },
        this.type,
        {
          next: resp => {
            this.router.navigate(['/supports/learning/smart-tracking-advanced/support/type']);
          },
          finally: () => {

...
import {
  SmartMotionDetectionSupport,
  SupportType,
  SupportFeature
} from '../../../../../../../vvtk-core/classes/support-smart-motion-detection';
...
export class SupportModelContentComponent implements OnInit {
  ...
  id: number;
  support: SmartMotionDetectionSupport;
  path = 'SmartMotionDetectionSupportModel';
  types: SupportType[] = [];
  ...
  getSupportModel() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartMotionDetection/Support/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        ...
        {
          next: resp => {
            this.router.navigate(['/supports/learning/smart-motion-detection/support/model']);
          },
          finally: () => {
            this.isLoading = false;
          }

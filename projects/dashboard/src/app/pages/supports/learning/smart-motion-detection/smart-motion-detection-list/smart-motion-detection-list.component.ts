...
@Component({
  selector: 'vvtk-smart-motion-detection-list',
  templateUrl: './smart-motion-detection-list.component.html',
  styleUrls: ['./smart-motion-detection-list.component.scss']
})
export class SmartMotionDetectionListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/supports/learning/smart-motion-detection/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/SmartMotionDetection/ManageList`
      },
      {
        next: resp => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
        ...
        this.vvtkService.delete(
          {
            path: `api/SmartMotionDetection/${id}`,
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/SmartMotionDetection/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

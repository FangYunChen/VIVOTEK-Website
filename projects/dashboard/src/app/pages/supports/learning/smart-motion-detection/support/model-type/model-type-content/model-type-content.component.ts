...
import { SupportType } from '../../../../../../../vvtk-core/classes/support-smart-motion-detection';
...
export class ModelTypeContentComponent implements OnInit {
  ...
  getModelType() {
    this.vvtkService.get(
      {
        path: `api/Products/SmartMotionDetection/Support/Type/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        {
          path: `api/Products/SmartMotionDetection/Support/Type`,
          disableLanguage: true
        },
        this.type,
        {
          next: resp => {
            this.router.navigate(['/supports/learning/smart-motion-detection/support/type']);
          },
          finally: () => {

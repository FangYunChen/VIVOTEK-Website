...
import { SupportTypeManage } from '../../../../../../../vvtk-core/classes/support-smart-motion-detection';
...
export class ModelTypeListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/supports/learning/smart-motion-detection/support/type/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/Products/SmartMotionDetection/Support/Type/ManageList`,
    ...
    dialogRef.afterClosed().subscribe((result: DeleteConfirmResult) => {
      if (result && result.value) {
        ...
        this.vvtkService.delete(
          {
            path: `api/Products/SmartMotionDetection/Support/Type/${id}/${result.value}`,
            disableLanguage: true
          },
          {
            next: resp => {
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/Products/SmartMotionDetection/Support/Type/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

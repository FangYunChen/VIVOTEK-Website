...
export class PackageListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/supports/learning/parking-violation-detection/support/package/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/Products/ParkingViolationDetection/Support/Package/ManageList`,
    ...
    dialogRef.afterClosed().subscribe((result: DeleteConfirmResult) => {
      if (!!result) {
        ...
        this.vvtkService.delete(
          {
            path: `api/Products/ParkingViolationDetection/Support/Package/${id}`,
            disableLanguage: true
          },
          {
            next: resp => {
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/Products/ParkingViolationDetection/Support/Package/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

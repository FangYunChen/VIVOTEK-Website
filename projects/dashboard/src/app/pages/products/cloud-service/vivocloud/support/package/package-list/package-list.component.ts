...
export class PackageListComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/products/cloud-service/vivocloud/support/package/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/Products/VIVOCloud/Support/Package/ManageList`,
    ...
    dialogRef.afterClosed().subscribe((result: DeleteConfirmResult) => {
      if (!!result) {
        ...
        this.vvtkService.delete(
          {
            path: `api/Products/VIVOCloud/Support/Package/${id}`,
            disableLanguage: true
          },
          {
            next: resp => {
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/Products/VIVOCloud/Support/Package/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

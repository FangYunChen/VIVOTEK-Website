...
import {
  ProductVIVOCloudSupport,
  SupportType,
  SupportStatus,
  SupportFeature
} from '../../../../../../../vvtk-core/classes/product-vivocloud';
...
export class SupportModelContentComponent implements OnInit {
  ...
  id: number;
  support: ProductVIVOCloudSupport;
  ...
  getSupportModel() {
    this.vvtkService.get(
      {
        path: `api/Products/VIVOCloud/Support/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        ...
        {
          next: resp => {
            this.router.navigate(['/products/cloud-service/vivocloud/support/model']);
          },
          finally: () => {
            this.isLoading = false;
          }

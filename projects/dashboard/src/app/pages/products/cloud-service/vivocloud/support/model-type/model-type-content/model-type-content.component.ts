...
export class ModelTypeContentComponent implements OnInit {
  ...
  getModelType() {
    this.vvtkService.get(
      {
        path: `api/Products/VIVOCloud/Support/Type/${this.id}`,
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        {
          path: `api/Products/VIVOCloud/Support/Type`,
          disableLanguage: true
        },
        this.type,
        {
          next: resp => {
            this.router.navigate(['/products/cloud-service/vivocloud/support/type']);
          },
          finally: () => {

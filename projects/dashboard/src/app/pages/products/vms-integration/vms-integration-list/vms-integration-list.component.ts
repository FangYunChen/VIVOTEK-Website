...
@Component({
  ...
  templateUrl: './vms-integration-list.component.html',
  styleUrls: ['./vms-integration-list.component.scss']
})
export class VMSintegrationListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/products/analytics/vms-plug-in/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/Product/VMSintegration/ManageList`
      },
      {
        next: resp => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
        ...
        this.vvtkService.delete(
          {
            path: `api/Product/VMSintegration/${id}`,
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/Product/VMSintegration/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

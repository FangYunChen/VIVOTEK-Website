...
@Component({
  selector: 'vvtk-facial-recognition-list',
  templateUrl: './facial-recognition-list.component.html',
  styleUrls: ['./facial-recognition-list.component.scss']
})
export class FacialRecognitionSolutionListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/solutions/application/facial-recognition-solution/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/Solutions/FacialRecognition/ManageList`
      },
      {
        next: resp => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
        ...
        this.vvtkService.delete(
          {
            path: `api/Solutions/FacialRecognition/${id}`,
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/Solutions/FacialRecognition/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

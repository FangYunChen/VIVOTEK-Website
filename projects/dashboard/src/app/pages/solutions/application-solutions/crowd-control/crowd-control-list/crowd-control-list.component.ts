...
@Component({
  ...
  templateUrl: './crowd-control-list.component.html',
  styleUrls: ['./crowd-control-list.component.scss']
})
export class CrowdControlSolutionListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/solutions/application/crowd-control-solution/0'
    );
  ...
  getData() {
    ...
    this.vvtkService.get(
      {
        path: `api/Solutions/CrowdControl/ManageList`
      },
      {
        next: resp => {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
        ...
        this.vvtkService.delete(
          {
            path: `api/Solutions/CrowdControl/${id}`,
  ...
  patchSequence() {
    ...
    this.vvtkService.patch(
      {
        path: `api/Solutions/CrowdControl/DisplayOrder`,
        disableLanguage: true
      },
      sequenceData,
      {

...
@Component({
  selector: 'vvtk-vadp-package-list',
  templateUrl: './vadp-package-list.component.html'
})
export class VADPPackageListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.pageContentIsEditable = this.sharedService.checkIsEditableByUrl(
      '/partners/sia-program/vadp/vadp-package/0'
    );
  ...
  private reloadData(filter: string, sort: Sort, page: PageEvent) {
    ...
    this.vvtkService.get(
      {
        path: `api/Partners/SIAProgram/VADP/VADPPackage/ManageList`,
  ...
  delete(id: number, companyName: string) {
    ...
    const dialogRef = this.dialog.open(ConfirmComponent, {
      ...
      data: {
        ...
        message: `Delete this company：${companyName} and packages?`
      }
    });
    dialogRef.afterClosed().subscribe(result => {
      if (!!result) {
        this.vvtkService.delete(
          {
            path: `api/Partners/SIAProgram/VADP/VADPPackage/${id}`,
            disableLanguage: true
          },
          {
            next: resp => {

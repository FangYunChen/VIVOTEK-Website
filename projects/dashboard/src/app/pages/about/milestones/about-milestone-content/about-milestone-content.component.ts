...
@Component({
  selector: 'vvtk-about-milestone-content',
  templateUrl: './about-milestone-content.component.html',
  styleUrls: ['./about-milestone-content.component.scss']
})
export class AboutMilestoneContentComponent implements OnInit, OnDestroy {
  ...
  save() {
    ...
    if (this.id === 0) {
      this.vvtkService.post(
        ...
        {
          next: resp => {
            this.router.navigate(['/about/milestones/list']);
          },
          finally: () => {
  ...
  imgFileSelect($event) {
    ...
    this.vvtkApiService.uploadFile(file, `About/Milestones/${file.name}`)
      .pipe(
        finalize(() => {
          this.isLoading = false;
